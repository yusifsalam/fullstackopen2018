...
class App extends React.Component {
  ...
  addPerson = event => {
    ...
    if (
      this.state.persons.filter(person => person.name === personObject.name)
        .length > 0
    ) {
      if (
        window.confirm(
  ...
  render() {
    return (
      <div>
        ...
        <h2>Lisää uusi</h2>
        <PersonForm
          personName={this.handlePersonName}
          newName={this.state.newName}
          personNumber={this.handlePersonNumber}
          ...
          {this.state.persons
            ...
            .map(person => (
              <Person
                key={person.name}
                person={person}
                deletePerson={this.handleDeletePerson(person.id, person.name)}
              />
            ))}
        </ul>
      </div>
